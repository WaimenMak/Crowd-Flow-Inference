# -*- coding: utf-8 -*-
# @Time    : 04/04/2025 10:46
# @Author  : mmai
# @FileName: GDN_LSTM_UQ
# @Software: PyCharm
...
class GCN_LSTM_UQ(nn.Module):
    def __init__(self, in_feats, h_feats, out_feats, lstm_hidden_size, num_layers, g):
        ...
        # fc for uncertainty
        self.sigma_fc = nn.ModuleList([nn.Linear(lstm_hidden_size, 32), nn.ReLU(), nn.Linear(32, g.num_nodes())])
    ...
    def forward(self, g, in_feat):
        ...
        h1_lst = []
        for i in range(in_feat.shape[-1]): # for every time step
        ...
        h, (hn, cn) = self.lstm(h) # h: [batch_size, num_timesteps_input, lstm_hidden_size]
        inpt_uq = h[:, -1, :]  # [batch_size, lstm_hidden_size]
        ...
        for layer in self.sigma_fc:
            inpt_uq = layer(inpt_uq)
        sigma = torch.log(1 + torch.exp(inpt_uq)) # [batch_zie, num_nodes]
        return h, sigma
...
if __name__ == '__main__':
    ...
    model = GCN_LSTM_UQ(in_feats=num_input_timesteps, h_feats=32, out_feats=pred_horizon-1, lstm_hidden_size=32, num_layers=2, g=g)  # out_size: prediction horizon
    ...
    for epoch in range(500):
        ...
        for i, (x, y) in enumerate(train_dataloader):
            ...
            first_step_label = g.ndata['label'][dst_idx, :, 0]
            pred, sigma = model(g, g.ndata['feature']) # [num_dst, batch_size]
            # loss = loss_fn(pred, y[:, 0, :])
            pred_first_step = pred[dst_idx, :, 0]
            loss_mse = loss_fn(pred[dst_idx, :, :], g.ndata['label'][dst_idx, :, :]) # [num_dst, batch_size], one-step prediction
            nll_loss = nll_loss_fn(pred_first_step.clamp(min=0), first_step_label, sigma[:, dst_idx].T)
            optimizer.zero_grad()
            loss = loss_mse + nll_loss
            loss.backward()
            optimizer.step()
            l.append(loss.item())
            # early stopping
    ...
    if dataset_name == "crossroad":
        torch.save(model.state_dict(), f'../checkpoint/gcn/gcnlstm_uq_crossroad_lags{lags}_hor{pred_horizon}.pth')
    elif dataset_name == "train_station":
        torch.save(model.state_dict(), f'../checkpoint/gcn/gcnlstm_uq_trainstation_lags{lags}_hor{pred_horizon}.pth')
    elif dataset_name == "maze":
        torch.save(model.state_dict(), f'../checkpoint/gcn/gcnlstm_uq_maze_lags{lags}_hor{pred_horizon}.pth')
    elif dataset_name == "edinburgh":
        torch.save(model.state_dict(), f'../checkpoint/gcn/gcnlstm_uq_edinburgh_lags{lags}_hor{pred_horizon}.pth')
