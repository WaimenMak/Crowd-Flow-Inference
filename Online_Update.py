...
if __name__ == '__main__':
    from Online_Models import (Online_Diffusion, Online_MA, ELM, Online_GCN, Online_GAT, Online_LSTM, Online_GCNLSTM,
                               Online_Diffusion_UQ, Online_Xgboost, Single_Model, Online_LSTM_Single, Online_Diffusion_plus)
    ...
    parser.add_argument('--chunk_size', type=int, default=105, help='Chunk size')
    parser.add_argument('--train_steps', type=int, default=0, help='Training iterations for each chunk')
    ...
    if args.model_type == 'Online_LSTM_Single':
        ...
        model = Single_Model(model_type=Online_Diffusion_plus, dataset=dataset_name, g=g, pred_horizon=pred_horizon, lags=args.lags, device=device,
                                 chunk_size=chunk_size, train_steps=args.train_steps, buffer=args.buffer)
