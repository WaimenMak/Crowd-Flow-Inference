...
class Probabilistic_Model(torch.nn.Module):
    def __init__(self, graph, num_edges, num_timesteps_input, hidden_units, scalar):
        ...
        self.scalar = scalar
        #params
        self.fc = nn.Linear(self.num_timesteps_input, hidden_units, bias=False)
        self.attn_fc = nn.Linear(2 * hidden_units, 1, bias=False)
        # self.ln = nn.LayerNorm(2 * hidden_units)
        # self.dropout = nn.Dropout(0.5)
...
if __name__ == '__main__':
    ...
    # out of distribution
    train_sc = [ 'sc_sensor/crossroad8']
    test_sc = ['sc_sensor/crossroad9']
